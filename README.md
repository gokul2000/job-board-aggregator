...
```bash
pip install jobhunt-board-aggregator
```
...
└── jobhunt/
    ...
    └── scrapers/
        ...
        ├── remoteok.py  # RemoteOK (JSON API, no auth)
        ├── linkedin.py  # LinkedIn (HTML scraping)
        ├── themuse.py   # The Muse (JSON API, no auth)
        ├── adzuna.py    # Adzuna (JSON API, free key)
        └── arbeitnow.py # Arbeitnow (JSON API, no auth)
```
...
| Command | Description |
|---|---|
| `jobhunt configure` | Set up job search preferences interactively |
| `jobhunt preferences` | List all saved preference profiles |
| `jobhunt preferences --delete ID` | Delete a preference by ID |
| `jobhunt search` | Search all job boards for all preferences |
| `jobhunt search --pref ID` | Search for a specific preference only |
| `jobhunt results` | View all saved job results |
| `jobhunt results --pref ID` | Filter results by preference |
| `jobhunt results --limit N` | Limit number of results shown |
| `jobhunt results --detail ID` | Show full details of a specific job |
| `jobhunt schedule` | Run periodic searches (default: every 60 min) |
| `jobhunt schedule --interval N` | Set search interval in minutes |
| `jobhunt email-config` | Configure SMTP for email notifications |
| `jobhunt -v <command>` | Run any command with verbose logging |
...
https://pypi.org/project/jobhunt-board-aggregator/
